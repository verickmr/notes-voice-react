      ...
      <Dialog.Portal>
        <Dialog.Overlay className="inset-0 fixed bg-black/50">
          <Dialog.Content className="overflow-hidden fixed -translate-x-1/2 left-1/2 top-1/2 -translate-y-1/2 max-w-[640px] w-full h-[60vh] bg-slate-700 rounded-md flex flex-col ">
            ...
            <button onClick={() => onNoteDeleted(note.id)} className="w-full bottom-0 bg-slate-800 py-4 text-center text-sm text-slate-400 outline-none font-medium group ">
              Deseja <span className="text-red-400 group-hover:underline">  apagar esse nota ?  </span>
            </button>
          </Dialog.Content>
        </Dialog.Overlay>
      </Dialog.Portal>
